...
INFILE=$1;shift
OUTDIR=$1;shift
SCRIPTDIR='/diskmnt/Projects/miRNA_scratch/Scripts/'
ANNDIR='/diskmnt/Projects/miRNA_scratch/Annotation/'
...
trimmomatic SE -threads 4 -trimlog ${f}_trimmomatic.log ${INDIR}/${f}.fastq.gz ${f}.trimmed.fastq ILLUMINACLIP:${ANNDIR}/Adapters.fa:0:30:1 SLIDINGWINDOW:10:20 MINLEN:15 HEADCROP:4 TAILCROP:4 AVGQUAL:30

###########################################################################################
### QC report before and after trimming
###########################################################################################

fastqc ${INDIR}/${f}.fastq.gz -o ${TMPDIR}
...
zcat ${INDIR}/${f}.fastq.gz | grep "@" | wc -l >> ${f}_report.txt
